...
export default function ConversionRecencyForm() {
  ...
  useEffect(() => {
    if (form.irCheckDate) {
      const checkDate = new Date(form.irCheckDate);
      ...
      if (validityDate < today) {
        setForm((prev) => ({
          ...prev,
          irCheckValidity: "OUT OF RECENCY",
        }));
      } else {
        setForm((prev) => ({
          ...prev,
  ...
  useEffect(() => {
    if (form.signalReceptionDate) {
      const checkDate = new Date(form.signalReceptionDate);
      const validityDate = new Date(checkDate);
      validityDate.setMonth(validityDate.getMonth() + 6);
      validityDate.setDate(validityDate.getDate() - 1);
      const today = new Date();
      ...
      if (validityDate < today) {
        setForm((prev) => ({
          ...prev,
          signalReceptionValidity: "OUT OF RECENCY",
        }));
      } else {
        setForm((prev) => ({
          ...prev,
    ...
    setSortieRows((prev) =>
      prev.map((row) => {
        if (row.id === id) {
          ...
          // Validate LDG and TO inputs (only 2 digits allowed)
          if (field === "ldg" || field === "to") {
            if (value && !/^\d{0,2}$/.test(value)) {
              return row; // Don't update if validation fails
            }
          }

          // Validate hours (2 digits)
          if (field === "hours") {
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <form
          ...
          className="bg-white shadow-2xl rounded-3xl p-8 sm:p-12 border border-gray-200 lg:w-[90%] mx-auto"
        >
          {status && (
            <div
                ...
                <div className="mb-8">
                            ...
                            <td className="border border-gray-300 px-2 py-2">
                              <div className="flex items-center gap-1">
                                <input
                                  ...
                                  placeholder="--"
                                  maxLength={2}
                                  className="w-12 px-2 py-1 border border-gray-300 rounded disabled:bg-gray-100 text-center"
                                />
                                <span>/</span>
                                <input
                                  type="text"
                  ...
                  <div className="mt-6 bg-blue-50 p-4 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                      ...
                      <div>
                        <p className="font-semibold">Total Night TO:</p>
                        <p>{sortieSummary.totalNightPICTO}</p>
                      </div>
                      <div>
                        <p className="font-semibold">Total Night LDG:</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    accept="application/pdf"
                    className={`w-full px-4 py-3 border-2 border-dashed rounded-xl focus:ring-2 focus:ring-blue-300 transition-all duration-200 ${
                      fileErrors.c172CheckrideStatement
                        ? "border-red-400 bg-red-50"
                        : "border-gray-300"
                    }`}
                    required
                  ...
                  {files.c172CheckrideStatement &&
                    !fileErrors.c172CheckrideStatement && (
                      <p className="text-green-600 text-sm mt-1">
                        ✓ {files.c172CheckrideStatement.name}
                      </p>
                    )}
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                {form.c172PICOption === "Flight Review / Check Ride on C172 in last 24 months" && (
                  <div className="mt-4">
                    ...
                    {files.c172FlightReview && !fileErrors.c172FlightReview && (
                      <p className="text-green-600 text-sm mt-1">
                        ✓ {files.c172FlightReview.name}
                      </p>
                    )}
                  </div>
                )}
          ...
          <section className="mb-12">
            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              <div>
                ...
                {files.pic100Statement && !fileErrors.pic100Statement && (
                  <p className="text-green-600 text-sm mt-1">
                    ✓ {files.pic100Statement.name}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                {files.crossCountry300Statement &&
                  !fileErrors.crossCountry300Statement && (
                    <p className="text-green-600 text-sm mt-1">
                      ✓ {files.crossCountry300Statement.name}
                    </p>
                  )}
              </div>
              ...
              <div>
                ...
                {files.picCrossCountryStatement &&
                  !fileErrors.picCrossCountryStatement && (
                    <p className="text-green-600 text-sm mt-1">
                      ✓ {files.picCrossCountryStatement.name}
                    </p>
                  )}
              </div>
              ...
              <div>
                ...
                {files.instrumentTimeStatement &&
                  !fileErrors.instrumentTimeStatement && (
                    <p className="text-green-600 text-sm mt-1">
                      ✓ {files.instrumentTimeStatement.name}
                    </p>
                  )}
              </div>
            </div>
          ...
          <section className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div>
                <label className="block text-lg font-bold text-gray-700 mb-2">
                  Validity (DD/MM/YYYY)<span className="text-red-500">*</span>
                </label>
                <input
                  type="date"
              ...
              <div>
                ...
                {files.medicalAssessment && !fileErrors.medicalAssessment && (
                  <p className="text-green-600 text-sm mt-1">
                    ✓ {files.medicalAssessment.name}
                  </p>
                )}
              </div>
            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          accept="application/pdf"
                          className={`w-full px-4 py-2 border-2 border-dashed rounded-lg focus:ring-2 focus:ring-blue-300 transition-all duration-200 ${
                            fileErrors[`dgcaExam_${detail.exam}`]
                              ? "border-red-400 bg-red-50"
                              : "border-gray-300"
                          }`}
                          required
                        ...
                        {files[`dgcaExam_${detail.exam}`] &&
                          !fileErrors[`dgcaExam_${detail.exam}`] && (
                            <p className="text-green-600 text-sm mt-1">
                              ✓ {files[`dgcaExam_${detail.exam}`].name}
                            </p>
                          )}
                      </div>
                    </div>
          ...
          <section className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                {files.rtrCertificate && !fileErrors.rtrCertificate && (
                  <p className="text-green-600 text-sm mt-1">
                    ✓ {files.rtrCertificate.name}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  type="file"
                  name="frtolCertificate"
                  accept="application/pdf"
                  className={`w-full px-4 py-3 border-2 border-dashed rounded-xl focus:ring-2 focus:ring-blue-300 transition-all duration-200 ${
                    fileErrors.frtolCertificate
                      ? "border-red-400 bg-red-50"
                      : "border-gray-300"
                  }`}
                  required
                ...
                {files.frtolCertificate && !fileErrors.frtolCertificate && (
                  <p className="text-green-600 text-sm mt-1">
                    ✓ {files.frtolCertificate.name}
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="block text-lg font-bold text-gray-700 mb-2">
                  Date of Police Verification (DD/MM/YYYY)
                  <span className="text-red-500">*</span>
                </label>
                <input
                  type="date"
              ...
              <div>
                ...
                {files.policeVerification && !fileErrors.policeVerification && (
                  <p className="text-green-600 text-sm mt-1">
                    ✓ {files.policeVerification.name}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                {files.marksheet10 && !fileErrors.marksheet10 && (
                  <p className="text-green-600 text-sm mt-1">
            ...
            {form.nameChangeProcessed === "Yes" && (
              <div className="mt-4">
                ...
                <input
                  ...
                  accept="application/pdf"
                  className={`w-full px-4 py-3 border-2 border-dashed rounded-xl focus:ring-2 focus:ring-blue-300 transition-all duration-200 ${
                    fileErrors.nameChangeCertificate
                      ? "border-red-400 bg-red-50"
                      : "border-gray-300"
                  }`}
                  required
                ...
                {files.nameChangeCertificate &&
                  !fileErrors.nameChangeCertificate && (
                    <p className="text-green-600 text-sm mt-1">
                      ✓ {files.nameChangeCertificate.name}
                    </p>
                  )}
              </div>
            )}
              ...
              <label className="flex items-start mt-4">
                ...
                <span className="text-gray-800 font-medium mb-4">
                  <p className="mb-2">
                    ...
                    operate in accordance with DGCA training and safety
                    standards. I further understand that flying training
                    involves inherent risks. Having fully understood these
                    risks, I voluntarily undertake sole responsibility for my
                    safety and for any incident or accident that may occur
                    during flight training for license conversion or recency,
                    and I agree that SkyPro Aviation shall not be held
                    responsible or liable in any manner.
                  </p>
                  <p>
                    I further understand and agree that if I fail to perform
                    satisfactorily during any checks, fail to obtain solo
                    release in the first attempt, or if there is any delay
                    arising due to my performance, skill level, preparedness, or
                    competency, then the 7-day commitment and 15-day completion
                    guarantee provided by SkyPro Aviation shall no longer be
                    applicable. Any additional training, time, or costs
                    resulting from such performance-related delays shall be my
                    sole responsibility.<span className="text-red-500">*</span>
                  </p>
                </span>
              </label>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
